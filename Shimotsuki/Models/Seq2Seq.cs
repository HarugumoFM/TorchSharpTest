...
namespace Shimotsuki.Models {
    public class Seq2Seq : Module {
        ...
        public Lang LangE;
        public Lang LangF;
        public Seq2Seq(int inputSize, int hiddenSize, int outputSize) : base("seq2seq") {
            LangE = new Lang();
            LangF = new Lang();
            this.hiddenSize = hiddenSize;
            this.encoder = new Encoder(inputSize, hiddenSize);
            this.decoder = new Decoder(hiddenSize * 2, outputSize);
            RegisterComponents();
        }
        ...
        public string evaluate(Tensor input, int maxLength) {
            ...
            for (int i = 0; i < maxLength; i++) {
                (var output, encoderHidden) = decoder.forward(input, encoderHidden);
        ...
        public void trainAll(List<string[]> pairs, int epoch) {
            ...
            double trainIter() {
                ...
                double train(Tensor input, Tensor target) {
                    ...
                    encOptim.step();
                    decOptim.step();
                    return loss.item<float>() / outputLength;
