...
{
    public class WordAttnSeq2Seq : Module
    {

        private WordEncoder encoder;
        private WordEncoder Reverse;
        private AttnDecoder decoder;
        private Embedding embedding;
        private Embedding posEmbedding;
        ...
        int hiddenSize;
        public Lang Lang;
        public WordAttnSeq2Seq(int inputSize, int hiddenSize, int maxToken) : base("seq2seq")
        {
            Lang = new Lang();
            this.embedding = Embedding(inputSize, hiddenSize);
            this.posEmbedding = Embedding(maxToken, hiddenSize);
            this.hiddenSize = hiddenSize;
            this.encoder = new WordEncoder(inputSize, hiddenSize, embedding,posEmbedding);
            this.Reverse = new WordEncoder(inputSize, hiddenSize, embedding,posEmbedding);
        ...
        public string evaluate(Tensor input, int maxLength)
        {
            ...
            {
                (var output, encoderHidden) = encoder.forward(input[i], encoderHidden, tensor(i));
                (var reverseOutput, reverseHidden) = Reverse.forward(input[inputLength - 1 - i], reverseHidden, tensor(inputLength - 1 -i));
